...
def predict(filename, mod, synsets, N=5):
    print 'PREDICTION TIME:', str(datetime.now())
    tic = time.time()
    ...
    img = img[np.newaxis, :]
    print "INFO: pre-processed image in "+str(time.time()-tic)
 
    toc = time.time()
    mod.forward(Batch([mx.nd.array(img)]))
    prob = mod.get_outputs()[0].asnumpy()
    prob = np.squeeze(prob)
    print "INFO: forward pass in "+str(time.time()-toc)
    ...
    a = np.argsort(prob)[::-1]
    for i in a[0:N]:
        topNstring.append('PREDICTION: probability=%f, class=%s' %(prob[i], synsets[i]))
        topN.append((prob[i], synsets[i]))
    return topN, topNstring
